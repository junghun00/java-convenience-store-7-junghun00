...
public class OutputView {
    ...
    private int printOrder(List<Receipt> receipts) {
        ...
        for (Receipt receipt : receipts) {
            totalOrder += receipt.getIndividualPrice() * receipt.getPromotionQuantity();
            System.out.printf(ORDER_LIST, receipt.getName(), receipt.getQuantity(), totalOrder);
        }
    ...
    private void printPaymentPrice(List<Receipt> receipts, int totalOrder, int totalPromotion, int discount) {
        ...
        System.out.printf("%-23s\t%d\n", "멤버십할인", discount);
        System.out.printf("%-23s\t%d\n", "내실돈", totalPromotion, (totalOrder - totalPromotion - discount));
    }
}